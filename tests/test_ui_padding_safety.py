...
class InstanceRuntimeTests(unittest.TestCase):
    def test_second_instance_can_signal_show_and_lock_recovers(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            mailbox = InstanceCommandMailbox(folder / "nova.command", owner_id=first.owner_id)
            self.assertFalse(second.acquire())
            self.assertTrue(mailbox.send("show", target_owner_id=first.owner_id))
            self.assertEqual(mailbox.consume(owner_id=first.owner_id)["command"], "show")
            first.release()
