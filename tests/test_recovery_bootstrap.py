...
class RecoveryFixture:
    ...
    def quarantine(self, remaining=None, **kwargs):
        return create_quarantine_journal(
            self.root, self.backup, backup_root=self.backup_root,
            remaining_processes=list(remaining or []), **kwargs,
        )

    def transaction(self):
        return create_transaction_journal(self.root, self.backup, backup_root=self.backup_root)
...
class StrongIdentityTests(unittest.TestCase):
    def test_matching_pid_and_creation_time_blocks(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(blocking, [identity]); self.assertEqual(errors, [])

    def test_same_pid_different_creation_time_does_not_block(self):
        with tempfile.TemporaryDirectory() as td:
            fx = RecoveryFixture(Path(td))
            ...
            self.assertEqual(blocking, []); self.assertEqual(errors, [])
    ...
    def test_inspection_error_is_conservative(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(len(blocking), 1); self.assertIn("access_denied", errors)

    def test_float_creation_time_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            fx = RecoveryFixture(Path(td))
...
class RecoveryStateMachineTests(unittest.TestCase):
    def test_schema_two_starts_at_transaction_prepared_before_mutation(self):
        with tempfile.TemporaryDirectory() as td:
    ...
    def test_explicit_graph_rejects_illegal_transition(self):
        with tempfile.TemporaryDirectory() as td:
            fx = RecoveryFixture(Path(td)); journal = fx.transaction()
            with self.assertRaises(RecoveryJournalError):
                transition_journal(fx.root, journal, "dependencies_running", backup_root=fx.backup_root)

    def test_stale_generation_cannot_overwrite_newer_state(self):
        with tempfile.TemporaryDirectory() as td:
            fx = RecoveryFixture(Path(td)); stale = fx.transaction()
            current = transition_journal(fx.root, stale, "files_applying", backup_root=fx.backup_root)
            self.assertGreater(current["generation"], stale["generation"])
            with self.assertRaises(StaleJournalWriterError):
                transition_journal(fx.root, stale, "rollback_in_progress", backup_root=fx.backup_root)

    def test_schema_one_migrates_fail_closed_and_persists_on_transition(self):
        with tempfile.TemporaryDirectory() as td:
            fx = RecoveryFixture(Path(td))
            payload = {
                "schema_version": 1, "attempt_id": "a" * 32, "generation": 3,
                "state": "validation_completed", "recovery_required": True,
                "backup_path": "attempt-1", "created_at": "x", "updated_at": "x",
                "dependencies_may_have_changed": False, "files_rollback_attempted": True,
                "files_rollback_ok": True, "remaining_processes": [], "errors": [],
            }
            ...
            current = transition_journal(fx.root, migrated, "cleared", backup_root=fx.backup_root)
            self.assertEqual(current["schema_version"], 2)


class RecoveryFlowTests(unittest.TestCase):
    def test_incomplete_transaction_restores_exact_files_validates_clears_and_launches_once(self):
        with tempfile.TemporaryDirectory() as td:
            fx = RecoveryFixture(Path(td)); journal = fx.transaction()
            journal = transition_journal(fx.root, fx.transaction(), "files_applying", backup_root=fx.backup_root, files_may_have_changed=True)
            ...
            self.assertTrue(result.recovered); self.assertTrue(result.launched)
            self.assertEqual(len(launches), 1); self.assertEqual(launches[0][0][-1], "--post-recovery")
            ...
            second = recover_pending(fx.root, backup_root=fx.backup_root, launcher=second_launch, launch_after_success=True)
            self.assertTrue(second.continue_startup); second_launch.assert_not_called()
    ...
    def test_crash_mid_rollback_resumes_idempotently(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            first = recover_pending(fx.root, backup_root=fx.backup_root, restore_func=partial, validator=lambda *_: (True, "ok"), launch_after_success=False)
            self.assertTrue(first.pending)
            self.assertEqual(load_journal(fx.root, backup_root=fx.backup_root)["state"], "rollback_in_progress")
            second = recover_pending(fx.root, backup_root=fx.backup_root, validator=lambda *_: (True, "ok"), launch_after_success=False)
            self.assertTrue(second.recovered); self.assertTrue(second.continue_startup)
            self.assertEqual(load_journal(fx.root, backup_root=fx.backup_root)["state"], "cleared")

    def test_dependency_validation_failure_enters_dependency_repair_required(self):
        with tempfile.TemporaryDirectory() as td:
            fx = RecoveryFixture(Path(td)); journal = fx.transaction()
            journal = transition_journal(fx.root, journal, "files_applying", backup_root=fx.backup_root, files_may_have_changed=True)
            journal = transition_journal(fx.root, journal, "files_applied", backup_root=fx.backup_root)
            journal = transition_journal(fx.root, journal, "dependencies_starting", backup_root=fx.backup_root, dependencies_may_have_changed=True)
            result = recover_pending(
                fx.root, backup_root=fx.backup_root,
    ...
    def test_non_dependency_validation_failure_keeps_rollback_validation_state(self):
        with tempfile.TemporaryDirectory() as td:
            fx = RecoveryFixture(Path(td)); journal = fx.transaction()
            journal = transition_journal(fx.root, journal, "files_applying", backup_root=fx.backup_root, files_may_have_changed=True)
            result = recover_pending(fx.root, backup_root=fx.backup_root, validator=lambda *_: (False, "syntax failed"), launch_after_success=False)
            self.assertTrue(result.pending)
            self.assertEqual(load_journal(fx.root, backup_root=fx.backup_root)["state"], "rollback_validation_in_progress")
            self.assertTrue(fx.backup.exists())


class RecoveryJournalSafetyTests(unittest.TestCase):
    def test_truncated_or_unknown_journal_fails_closed(self):
        for text in ('{"schema_version":2,"attempt_id š', json.dumps({"schema_version": 99})):
            with self.subTest(text=text[:20]), tempfile.TemporaryDirectory() as td:
                fx = RecoveryFixture(Path(td)); journal_path(fx.root).write_text(text, encoding="utf-8")
                result = recover_pending(fx.root, backup_root=fx.backup_root, launch_after_success=False)
                self.assertTrue(result.pending); self.assertEqual(result.exit_code, RECOVERY_REQUIRED_EXIT_CODE)

    def test_backup_traversal_and_symlink_escape_are_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            fx = RecoveryFixture(Path(td))
            payload = {
                "schema_version": 2, "attempt_id": "a" * 32, "generation": 1,
                "state": "waiting_for_processes", "recovery_required": True,
                "backup_path": "../outside", "remaining_processes": [], "errors": [],
            }
            journal_path(fx.root).write_text(json.dumps(payload), encoding="utf-8")
            with self.assertRaises(RecoveryJournalError): load_journal(fx.root, backup_root=fx.backup_root)
        with tempfile.TemporaryDirectory() as td:
            base = Path(td); fx = RecoveryFixture(base); outside = base / "outside"; outside.mkdir(); (outside / "backup.json").write_text("{}")
            ...
            payload = {
                "schema_version": 2, "attempt_id": "a" * 32, "generation": 1,
                "state": "waiting_for_processes", "recovery_required": True,
                "backup_path": "escape", "remaining_processes": [], "errors": [],
            }
            journal_path(fx.root).write_text(json.dumps(payload), encoding="utf-8")
            journal = load_journal(fx.root, backup_root=fx.backup_root)
            with self.assertRaises(RecoveryJournalError): resolve_backup(fx.root, journal, backup_root=fx.backup_root)

    def test_updater_gate_is_read_only_while_strong_process_is_alive(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = updater_recovery_gate(fx.root, backup_root=fx.backup_root, inspector=lambda row: ("alive", ""))
