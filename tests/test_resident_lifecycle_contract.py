...
class Warm:
    ...
    def unload(self, **_kwargs): self.unload_calls += 1

class Tray(Service):
    available = True
    degraded = False
    def status(self): return {"available": self.available, "degraded": self.degraded}
...
class UI:
    def __init__(self):
        self._closing = False; self._accepting_commands = True; self._recording = False
        self._wake_stop = WakeStop(); self._wake_thread = None
        self._hotkey_listener = Service(); self._ptt_listener = Service()
        self.gaming_awareness = Service(); self.perception = Service(); self.llm_warm_manager = Warm()
...
class ResidentLifecycleContractTests(unittest.TestCase):
    def make(self):
        ...
        manager = RuntimeLifecycleManager(root, {"resident_mode": {"enabled": True, "close_to_tray": True}}, ui=ui, scheduler=lambda callback: callback(), data_root=Path(temp.name))
    ...
    def test_hidden_runtime_keeps_wake_hotkeys_gaming_perception_and_qwen(self):
        manager, root, ui, _tray, _instance = self.make(); manager.request_window_close()
        self.assertEqual(manager.state, "hidden"); self.assertTrue(root.hidden)
        self.assertEqual(ui._wake_stop.set_calls, 0); self.assertEqual(ui._hotkey_listener.stop_calls, 0)
        self.assertEqual(ui._ptt_listener.stop_calls, 0); self.assertEqual(ui.gaming_awareness.stop_calls, 0)
        self.assertEqual(ui.perception.stop_calls, 0); self.assertEqual(ui.llm_warm_manager.unload_calls, 0)

    def test_tray_exit_keeps_physical_runtime_lock_until_app_finalizer(self):
        manager, root, ui, tray, instance = self.make()
        self.assertTrue(manager.request_shutdown("tray_exit")); self.assertFalse(manager.request_shutdown("tray_exit"))
