...
- Pip se inicia en una unidad identificable: nueva sesión/grupo en Unix y nuevo process group en Windows. La terminación usa grupo de procesos en Unix y `psutil` recursivo en Windows, con snapshot, terminate, espera acotada, re-scan, kill de restantes, wait/reap final y verificación de procesos vivos.
- La terminación devuelve un resultado estructurado con `terminated_confirmed`, `direct_process_terminated`, `remaining_pids`, `termination_errors` y `detail`; `kill()` por sí solo nunca se considera prueba de salida.
- El mensaje “pip y sus descendientes fueron detenidos y esperados de forma verificable” solo se emite cuando la inspección final confirma que el proceso directo terminó y no quedan descendientes conocidos vivos.
- Con terminación confirmada, el timeout se considera mutación de dependencias iniciada: se ejecuta rollback de archivos, se conserva el backup y se persisten `files_rollback_ok`, `dependencies_may_have_changed=true` y `recovery_required=true`.
- Si la terminación no puede confirmarse tras la escalada normal y forzada, la actualización entra en fail-closed con código `6`: no inicia rollback concurrente, no declara `files_rollback_ok=true`, conserva backup/manifiesto, escribe `status=pip_termination_unconfirmed`, registra solo PID restantes necesarios y no relanza Nova desde la misma `.venv`.
...
- Nova Doctor añade el estado del updater residente: supervisor activo/inactivo según el mutex del kernel, último resultado disponible, recuperación pendiente y terminación de pip no confirmada; los PID restantes solo se muestran cuando esa recuperación los necesita.
...
- Se añaden pruebas de doble clic y seguimiento Tk no bloqueante; mutex de supervisor y orden de locks; dos supervisores reales simultáneos en Windows y recuperación del mutex tras morir el propietario; rollback con archivos reales y SHA-256; terminate/kill/verificación del árbol de pip sin sleeps reales; fail-closed sin rollback ante terminación no confirmada; diagnóstico minimizado; y las regresiones previas de lifecycle/IPC/bandeja.
- CI mantiene la suite completa Ubuntu y ejecuta explícitamente en `windows-latest` lifecycle, bandeja, rollback, IPC/procesos, updater, pruebas multiproceso del mutex, session shutdown, Gaming Awareness, Instant Wake/hotkeys y núcleo nativo.
