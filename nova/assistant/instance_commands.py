...
class InstanceCommandMailbox:
    ...
    def _read_message(self, path: Path) -> tuple[dict[str, Any] | None, str]:
        try:
            ...
            if not isinstance(data, dict): return None, "invalid_message"
            required = {"command_id", "target_owner_id", "command", "created_at"}
            if set(data) != required: return None, "invalid_message"
            ...
            if len(command_id) < 16 or created <= 0: return None, "invalid_message"
            if abs(time.time() - created) > self.max_age_seconds: return None, "stale_message"
            return {"command_id": command_id, "target_owner_id": target, "command": command, "created_at": created}, ""
    ...
    def consume(self, *, owner_id: str | None = None) -> dict[str, Any] | None:
        ...
        for path in paths:
            ...
            except OSError: return {"ok": False, "error": "claim_failed"}
            return {"ok": True, **data}
        if first_error: return {"ok": False, "error": first_error}
        return None
