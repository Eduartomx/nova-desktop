...
"""Anomaly Detection local y contextual para Nova.

Aprende una linea base de CPU/RAM por actividad y de consumo por proceso, y solo
emite eventos tras desviaciones sostenidas. No usa LLM, no inspecciona contenido,
no lee cmdline/rutas y nunca mata o modifica procesos automaticamente.
"""
...
def _stats(samples: int, mean: float, m2: float) -> dict[str, float | int]:
    std = math.sqrt(max(0.0, m2 / max(1, samples - 1))) if samples > 1 else 0.0
    return {"samples": int(samples), "mean": round(mean, 3), "std": round(std, 3)}


def _update_welford(samples: int, mean: float, m2: float, value: float) -> tuple[int, float, float]:
    n = int(samples) + 1
    delta = value - mean
...
class AnomalyDetector:
    ...
    def _init_db(self):
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_anomaly_events_ack ON anomaly_events(acknowledged, severity);
    ...
    def _sample_processes(self) -> list[dict[str, Any]]:
        ...
        current_state: dict[int, tuple[float, float, float]] = {}
        ...
        for proc in psutil.process_iter(["pid", "name", "memory_info", "create_time"]):
            try:
                ...
                cpu_percent = 0.0
                if previous and abs(previous[2] - create_time) < 0.001:
                    delta_wall = max(0.001, now - previous[1])
                    cpu_percent = max(0.0, (cpu_total - previous[0]) / delta_wall * 100.0 / cpu_count)
                current_state[pid] = (cpu_total, now, create_time)
                rows.append(
                    {
                        "pid": pid,
                        "name": name,
                ...
                continue
        self._proc_cpu_state = current_state
        rows.sort(key=lambda row: max(_float(row.get("cpu_percent")), _float(row.get("memory_percent")) * 4.0), reverse=True)
        return rows[:80]
    ...
    def _context(self) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        activity_key = _norm(activity.get("activity"))
        app_kind = _norm(external.get("app_kind"))
        context_key = activity_key or app_kind or "desktop"
        return context_key[:80], state, snap

    def _baseline(self, context_key: str) -> dict[str, Any]:
        with self._lock, self._connect() as conn:
            row = conn.execute("SELECT * FROM anomaly_baselines WHERE context_key=?", (context_key,)).fetchone()
        if not row:
            return {"samples": 0, "cpu": _stats(0, 0, 0), "memory": _stats(0, 0, 0)}
        return {
            "samples": int(row["samples"]),
            "cpu": _stats(int(row["samples"]), float(row["cpu_mean"]), float(row["cpu_m2"])),
            "memory": _stats(int(row["samples"]), float(row["memory_mean"]), float(row["memory_m2"])),
        }

    def _process_baseline(self, context_key: str, process_name: str) -> dict[str, Any]:
        with self._lock, self._connect() as conn:
            row = conn.execute(
    ...
    def _learn_system(self, context_key: str, cpu: float, memory: float):
        with self._lock, self._connect() as conn:
            row = conn.execute("SELECT * FROM anomaly_baselines WHERE context_key=?", (context_key,)).fetchone()
            if row:
                n1, cpu_mean, cpu_m2 = _update_welford(int(row["samples"]), float(row["cpu_mean"]), float(row["cpu_m2"]), cpu)
                n2, mem_mean, mem_m2 = _update_welford(int(row["samples"]), float(row["memory_mean"]), float(row["memory_m2"]), memory)
                conn.execute(
                    ...
                    (min(n1, n2), cpu_mean, cpu_m2, mem_mean, mem_m2, context_key),
                )
            else:
                conn.execute(
                    "INSERT INTO anomaly_baselines(context_key,samples,cpu_mean,memory_mean) VALUES (?,?,?,?,?)".replace("VALUES (?,?,?,?,?)", "VALUES (?,?,?,?)"),
                    (context_key, 1, cpu, memory),
                )
            conn.commit()

    def _learn_process(self, context_key: str, process_name: str, cpu: float, memory: float):
        ...
        with self._lock, self._connect() as conn:
            ...
            if row:
                ...
                conn.execute(
                    ...
                    (min(n1, n2), cpu_mean, cpu_m2, mem_mean, mem_m2, context_key, name),
                )
            else:
                conn.execute(
    ...
    def _expected_processes(self, state: dict[str, Any]) -> set[str]:
        ...
        expected.update(_norm(row["process_name"]) for row in rows)
        return expected

    def _limits(self, context_key: str, baseline: dict[str, Any], expected_heavy: bool = False) -> tuple[float, float]:
        sigma = max(1.0, _float(self.config.get("sigma_threshold"), 3.0))
        cpu_floor = _float(self.config.get("system_cpu_floor"), 82.0)
        mem_floor = _float(self.config.get("system_memory_floor"), 86.0)
        ...
        cpu_stats = baseline.get("cpu") or {}
        mem_stats = baseline.get("memory") or {}
        cpu_limit = max(
            cpu_floor,
            _float(cpu_stats.get("mean")) + max(_float(self.config.get("system_cpu_min_delta"), 18.0), sigma * _float(cpu_stats.get("std"))),
        )
        mem_limit = max(
            mem_floor,
            _float(mem_stats.get("mean")) + max(_float(self.config.get("system_memory_min_delta"), 12.0), sigma * _float(mem_stats.get("std"))),
        )
    ...
    def _process_limits(self, baseline: dict[str, Any]) -> tuple[float, float]:
        ...
        mem_stats = baseline.get("memory") or {}
        cpu_limit = max(
            _float(self.config.get("process_cpu_floor"), 25.0),
            _float(cpu_stats.get("mean")) + max(_float(self.config.get("process_cpu_min_delta"), 12.0), sigma * _float(cpu_stats.get("std"))),
        )
        mem_limit = max(
            _float(self.config.get("process_memory_floor"), 8.0),
            _float(mem_stats.get("mean")) + max(_float(self.config.get("process_memory_min_delta"), 5.0), sigma * _float(mem_stats.get("std"))),
        )
    ...
    def _emit(self, event_type: str, severity: str, context_key: str, process_name: str = "", score: float = 0.0, metadata: dict[str, Any] | None = None):
        ...
        now = time.monotonic()
        cooldown = max(10.0, _float(self.config.get("event_cooldown_seconds"), 300.0))
        if now - self._last_emit_at.get(signature, 0.0) < cooldown:
            return None
        ...
        with self._lock, self._connect() as conn:
            cur = conn.execute(
                ...
                (str(event_type)[:80], str(severity)[:16], context_key[:80], _norm(process_name)[:120], float(score), json.dumps(safe, ensure_ascii=False, separators=(",", ":"))),
            )
    ...
    def _crash_signals(self, processes: list[dict[str, Any]], context_key: str) -> list[dict[str, Any]]:
        ...
        for row in processes:
            ...
            if name not in crash_names or not pid or pid in self._seen_crash_pids:
                continue
            self._seen_crash_pids.add(pid)
            with self._lock, self._connect() as conn:
                recent = conn.execute(
                    "SELECT COUNT(*) AS n FROM anomaly_events WHERE event_type='crash_signal' AND created_at >= datetime('now','-15 minutes')"
                ).fetchone()
            repeated = int(recent["n"] if recent else 0) >= 1
            event = self._emit(
                "crash_signal",
                "high" if repeated else "warn",
                context_key,
                name,
                0.95 if repeated else 0.65,
                {"repeated_15m": repeated},
            )
            if event:
    ...
    def sample_once(self) -> dict[str, Any]:
        ...
        cpu = _float(system.get("cpu_percent"))
        memory = _float(system.get("memory_percent"))
        processes = list(self.process_sensor() or [])
        expected = self._expected_processes(state)
        expected_heavy_present = any(_norm(row.get("name")) in expected and _float(row.get("cpu_percent")) >= 15 for row in processes)

        baseline = self._baseline(context_key)
        ready = int(baseline.get("samples") or 0) >= max(3, int(self.config.get("baseline_min_samples", 24)))
        cpu_limit, mem_limit = self._limits(context_key, baseline, expected_heavy_present)
        cpu_candidate = ready and cpu >= cpu_limit
        mem_candidate = ready and memory >= mem_limit
        emitted: list[dict[str, Any]] = self._crash_signals(processes, context_key)
        ...
        if not ready or (not cpu_candidate and not mem_candidate):
            self._learn_system(context_key, cpu, memory)

        process_candidates = 0
        for row in processes[:40]:
            name = _norm(row.get("name"))
            if not name or name in {_norm(x) for x in self.config.get("crash_signal_processes", [])}:
                continue
            p_cpu = _float(row.get("cpu_percent"))
            p_mem = _float(row.get("memory_percent"))
            pbase = self._process_baseline(context_key, name)
            p_ready = int(pbase.get("samples") or 0) >= max(3, int(self.config.get("process_baseline_min_samples", 12)))
            p_cpu_limit, p_mem_limit = self._process_limits(pbase)
            if p_ready:
                candidate = (p_cpu >= p_cpu_limit or p_mem >= p_mem_limit) and name not in expected
            else:
                candidate = (
                    p_cpu >= _float(self.config.get("new_process_cpu_threshold"), 35.0)
                    or p_mem >= _float(self.config.get("new_process_memory_threshold"), 10.0)
                ) and name not in expected
            if candidate:
                process_candidates += 1
            sustained = self._sustained(f"process:{context_key}:{name}", candidate)
            if sustained:
                severity = "high" if p_cpu >= 55 or p_mem >= 16 else "warn"
                event = self._emit(
                    "process_resource_anomaly",
                    severity,
                    context_key,
                    name,
                    score=min(1.0, max(p_cpu / max(1.0, p_cpu_limit), p_mem / max(0.1, p_mem_limit))),
                    metadata={
                        "cpu_percent": round(p_cpu, 1),
                        "memory_percent": round(p_mem, 2),
                        "baseline_samples": int(pbase.get("samples") or 0),
                        "new_process": not p_ready,
                    },
                )
                if event:
                    emitted.append(event)
    ...
    def acknowledge(self, event_id: int | None = None) -> dict[str, Any]:
        with self._lock, self._connect() as conn:
            if event_id is None:
                cur = conn.execute("UPDATE anomaly_events SET acknowledged=1 WHERE acknowledged=0")
            else:
                cur = conn.execute("UPDATE anomaly_events SET acknowledged=1 WHERE id=?", (int(event_id),))
            conn.commit()
    ...
    def compact_context(self) -> str:
        ...
        lines = [
            f"Anomaly Detection: {'baseline listo' if status.get('baseline_ready') else 'aprendiendo baseline'} · contexto {status.get('context_key')} · {status.get('baseline_samples')}/{status.get('baseline_min_samples')} muestras.",
        ]
        if rows:
            labels = []
            for row in rows[:3]:
                process = row.get("process_name") or "sistema"
                labels.append(f"{row.get('severity')}:{row.get('event_type')}:{process}")
    ...
    def format_recent(self, limit: int = 10) -> str:
        ...
        for row in rows:
            ...
            lines.append(f"- [{str(row.get('severity')).upper()}] {row.get('event_type')} · {process}{detail}")
        return "\n".join(lines)
    ...
    def _loop(self):
        while not self._stop.is_set():
            try:
                ...
                pass
            self._stop.wait(max(1.0, _float(self.config.get("sample_interval_seconds"), 5.0)))
...
def get_anomaly_detector(config: dict[str, Any] | None = None, memory=None) -> AnomalyDetector:
    ...
    key = id(engine)
    anomaly_cfg = cfg.get("anomaly_detection", {}) if isinstance(cfg, dict) else {}
    detector = _instances.get(key)
    if detector is None:
        detector = AnomalyDetector(engine, intelligence, memory=memory, config=anomaly_cfg)
        _instances[key] = detector
    else:
        detector.configure(anomaly_cfg).attach_memory(memory)
